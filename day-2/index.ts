...
const getMoves = (round: string): [OPPONENT_MOVES, MY_MOVES] =>
  round.split(" ") as [OPPONENT_MOVES, MY_MOVES];
...
const calculatePartBScore = (input: string): number => {
  ...
  const getRequiredMove = (
    opponentMove: OPPONENT_MOVES,
    requiredResult: MY_MOVES,
  ): MY_MOVES => {
    // X = need to lose
...
const partBAnswer = await calculatePartBScore(
