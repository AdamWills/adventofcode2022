...
const doAllItemsExist = (x: number[], y: number[]): boolean => x.every((item) => y.includes(item));
const doAnyItemsOverlap = (x: number[], y: number[]): boolean => x.some((item) => y.includes(item));
...
export { calculateOverlappingPairs, calculateAnyOverlappingPairs };
